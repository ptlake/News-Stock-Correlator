...
while year != y_end or month != m_end:
    ...
    if os.path.exists(csv):
        ...
        if r.status_code != 200:
            raise urllib.error.URLError(f"Problem accessing NYT\nSTATUS CODE:  {r.status_code}")
    ...
    response = mongo_collection.find_one({'month':date_str},projection={'month':True})
    if not response:
        print("  Uploading data")
        articles = df[['pub_date','abstract','lead_paragraph','headline.main','section_name']].to_dict(orient='list')
        d = {'month':date_str,'n_articles':len(articles['pub_date']),'articles':articles}
